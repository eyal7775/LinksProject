...
class CSVFormat(ILinks):
    ...
    def create_library_sources(self):
        ...
        self.file_path = "sources/" + name_source + "/" + name_source + "_md" + str(self.max_depth) + "_" + timestamp + ".csv"

    # create csv file
    def create_file_format(self):
        header = ["serial", "link", "depth", "access"]
        with open(self.file_path, "w", encoding='utf8', newline='\n') as file:
            csv_writer = csv.writer(file, delimiter=',', quoting=csv.QUOTE_ALL)
            csv_writer.writerow(header)

    # insert url data to file results if exist
    def add_data_to_csv(self, link, depth):
    ...
    def write_to_file(self, data):
        with open(self.file_path, "a+", encoding='utf8', newline='\n') as file:
            csv_writer = csv.writer(file, delimiter=',', quoting=csv.QUOTE_ALL)
        ...
        if depth > self.max_depth:
            ...
            for link in links:
                if not link.split('.')[-1] in ignore:
                    self.add_data_to_csv(link, depth)
